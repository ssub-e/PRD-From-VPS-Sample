# B2B 수요예측 AI SaaS — PRD v0.3

- **Owner 팀:** Product & AI Engineering
- **최종 업데이트:** 2026-04-15
- **VPS 원본 참조:** `04_VPS-final/06_VPS-Integrated-V2.md`
- **문서 상태:** Draft — 최종 검토 완료 (6개 패스 항목 FULL PASS)
- **변경 이력:**
  - v0.1 → v0.2: 리뷰 보고서(`PRD-v0.1-review.md`) 기반 15개 갭 수정 적용
  - v0.2 → v0.3: 최종 검토 2건 보완 (Story 3 Failure AC 추가, ADR 섹션 신설)
...
| **AC-6 (Failure)** | **Failure** | 특정 화주사의 API 연동이 끊기거나(토큰 만료/API 에러) 데이터 수집이 중단될 때 | 시스템이 연동 실패를 감지하면 | ① 해당 화주사에 "연동 갱신 필요" 알림 발송 ② 센터장에게 "N개 화주 데이터 미수집" 경고 표시 ③ 미수집 화주를 제외한 나머지 데이터로 예측 계속 동작 | 연동 실패 감지 → 알림 발송 **≤ 10분**, 부분 예측 계속 동작율 **100%** |
...
| **AC-5 (Failure)** | 차량 GPS 신호가 끊기거나(터널/지하) 위치 데이터 수신이 중단될 때 | 시스템이 GPS 유실을 감지하면 | ① 마지막 유효 위치 + 캐시 온도 데이터 기반으로 보수적 노선 재설정 유지 ② 기사에게 "GPS 불안정 — 수동 위치 확인 요청" 알림 발송 ③ 관리자 대시보드에 "GPS 유실 구간" 경고 표시 | GPS 유실 감지 → 알림 **≤ 30초**, 보수적 노선 유지율 **100%** |
| **AC-6 (Failure)** | IoT 온도 센서 데이터 수신이 지연(>1분)되거나 센서 이상값(예: -50°C)이 감지될 때 | 시스템이 센서 이상을 감지하면 | ① 관리자에게 즉시 "온도 센서 이상" 긴급 알림 ② 직전 정상 온도 데이터 기반 예측 유지 ③ 해당 차량의 노선 재설정을 "인간 승인 필수" 모드로 전환 | 센서 이상 감지 → 알림 **≤ 10초**, 인간 승인 모드 전환율 **100%** |
...
| 인프라 월 비용 (MVP 기간) | **≤ 500만 원** (AWS 스타트업 크레딧 활용 — VPS §1-4 KSF #5) | AWS Cost Explorer 월간 리포트 검토 |
| 예측 엔진 코어 IP 보호 | SaaS API 과금 모델로 코어 알고리즘 방어 *(VPS §1-3 기술 자산 보호)* | API 엔드포인트 리버스 엔지니어링 감사 (연 1회) |
...
## 7-5. ADR (Architecture Decision Records)

> 제품 구조의 핵심 설계 결정을 기록·추적합니다.

### ADR-001: 예측 모델 자체 개발 (Build) vs 외부 MLaaS (Buy)

| 항목 | 내용 |
|---|---|
| **상태** | ✅ Accepted |
| **컨텍스트** | 이커머스 계절성·프로모션·날씨 등 도메인 특화 변수를 반영한 시계열 예측이 핵심 차별가치. 범용 MLaaS(AWS Forecast, GCP Vertex AI)는 도메인 커스터마이징에 한계. |
| **결정** | TFT/N-BEATS 기반 예측 모델을 **100% 자체 개발(Build)**한다. |
| **근거** | KSF #4 "버티컬 도메인 특화 알고리즘 100% 자체 IP화" *(VPS §1-4)*. 코어 알고리즘이 경쟁 우위의 원천. 외주 시 IP 보호 불가. |
| **결과** | Phase 2에 2주 모델 학습 기간 확보. PyTorch 기반 앙상블 아키텍처. 합성 데이터 + 전이학습으로 초기 데이터 부족 대응 (R3 리스크). |
| **대안 기각 사유** | AWS Forecast: SHAP 해설 커스터마이징 불가, 이커머스 프로모션 변수 직접 주입 어려움. 마키나락스 MLOps: 초기 구축 억 단위 비용 + 6~12개월 리드타임 *(VPS §1-3)*. |
...
|---|---|
| **상태** | ✅ Accepted |
| **컨텍스트** | 화주사 데이터(매출·재고·프로모션)는 B2B 고객 간 절대 교차 접근 불가. 그러나 MVP 단계에서 고객 수 ≤ 20으로 물리 격리(DB 인스턴스 분리)는 비용 과다. |
| **결정** | 테넌트별 **논리 격리(DB 스키마 분리)**를 적용한다. |
| **근거** | 인프라 월 비용 ≤ 500만 원 제약 *(§5-3, VPS §1-4 KSF #5)*. MVP 단계 고객 ≤ 20곳에서 스키마 분리로 충분한 보안 수준 확보 가능. |
| **결과** | PostgreSQL 스키마 기반 멀티테넌시. 릴리스마다 교차 테넌트 접근 테스트 실행 *(§5-3)*. 고객 50곳 초과 시 물리 격리 재검토 트리거. |
| **대안 기각 사유** | 물리 격리(DB 인스턴스 per 테넌트): MVP 단계 비용 3~5배 증가. 행 수준 보안(RLS): 단일 스키마에서 개발자 실수로 교차 접근 가능성 높음. |

---

### ADR-003: PDF 렌더링 Puppeteer vs 서버사이드 LaTeX

| 항목 | 내용 |
|---|---|
| **상태** | ✅ Accepted |
| **컨텍스트** | 경영진 결재용 PDF 리포트는 기존 엑셀 보고 양식과 호환되는 레이아웃 필요 *(Story 1 AC-5)*. PDF 생성 방식으로 Puppeteer(헤드리스 브라우저)와 LaTeX/WeasyPrint가 후보. |
| **결정** | **Puppeteer 기반 PDF 렌더링**을 적용한다. |
| **근거** | React 대시보드와 동일한 컴포넌트로 PDF 생성 가능 → 개발 비용 최소화. 파일럿 고객별 결재 양식 커스터마이징이 HTML/CSS 수준에서 가능. |
| **결과** | Buy 판정 *(§4-2)*. 렌더링 타임아웃 > 3분 시 자동 재시도 2회 + 웹 대시보드 폴백 *(Story 1 AC-7)*. |
| **대안 기각 사유** | LaTeX: 한국어 폰트/레이아웃 커스터마이징 러닝커브 높음, 파일럿 고객별 양식 대응 비효율. WeasyPrint: 차트/그래프 렌더링 품질 부족. |
...
|---|---|---|---|---|---|---|---|
| **PoC-1: XAI 리포트 결재율** | XAI 해설이 포함된 PDF 리포트는 기존 엑셀 보고서 대비 경영진 결재 통과율을 높인다 | 1-pass 결재율, 기회손실 변동 | 결재율 ≥ **80%**, 기회손실 ≤ 기준선의 **10%** | 파일럿 **2사** × MD **2~3인** = **최소 4인**, 발주 결재 건 **≥ 40건** | **4주** (Phase 3) | Before/After paired t-test, p < 0.05 | S1: AC-1, AC-2, AC-4 |
| **PoC-2: 인건비 절감** | API 연동 + 인원 역산 대시보드가 3PL 센터의 오버 스케줄링을 감소시킨다 | 인건비 절감률, 인원 오차율 | 절감률 ≥ **30%**, 오차율 ≤ **5%** | 파일럿 물류센터 **1~2곳**, 운영일 **≥ 20일** | **4주** (Phase 3) | Before/After 일별 비교, Wilcoxon signed-rank test | S2: AC-1, AC-2, AC-5 |
| **PoC-3: 연동 편의성** | 1-click OAuth 연동이 화주사의 자발적 참여를 유도한다 | 연동 성공률, 연동 소요시간 | 성공률 ≥ **95%**, 소요시간 ≤ **1분** | 파일럿 센터 소속 화주사 **≥ 10곳** | **2주** (Phase 3 초) | 성공/실패 비율 단순 집계 | S2: AC-4 |
| **EXP-04: MAPE 벤치마크** | 우리 MAPE가 기존 엑셀 MAPE보다 유의하게 낮다 | 예측 MAPE | 우리 MAPE **≤ 25%** 이고 기존 대비 **30%+ 개선** | 파일럿 고객 **2사** SKU **≥ 50개** × **4주** 일별 예측 | Phase 2~3 | paired t-test on daily MAPE, p < 0.05 | S1: AC-3 |
| **A/B Test** *(Post-MVP)* | XAI 해설 유무에 따른 사용자 신뢰도 차이 측정 | 만족도(5점), 신뢰 점수 | XAI 그룹 ≥ **4.0** (대조군 대비 **+1.0**) | **n ≥ 64** per group (효과크기 d=0.5, 검정력 80%, α=0.05 기준 산출) | Post-MVP **4주** | 독립표본 t-test, p < 0.05 | S1: AC-4 |
...
*End of PRD v0.3 — Draft (최종 검토 완료, 6개 패스 항목 FULL PASS)*
